<template>
  ...
  <el-form size="mini" :inline="true"  class="query-form" ref="searchForm" :model="searchForm"  @keyup.enter.native="refreshList()" @submit.native.prevent>
         <el-form-item prop="name">
          <el-input size="mini"  v-model="searchForm.name"   placeholder="菜单名称"   clearable></el-input>
        </el-form-item>
      <el-form-item>
    ...
    <vxe-toolbar :refresh="{query: refreshList}" export print custom>
      <template #buttons>
        <el-button v-if="hasPermission('sys:menu:add')" type="primary" icon="el-icon-plus" size="mini" @click="add()">新增</el-button>
        <el-button v-if="hasPermission('sys:menu:del')" :disabled="$refs.xTree && $refs.xTree.getCheckboxRecords().length === 0" type="danger" size="mini" icon="el-icon-delete" @click="del()">删除</el-button>
      </template>
    </vxe-toolbar>
    <div style="height: calc(100% - 75px);">
      <vxe-table
        ...
        <vxe-table-column  title="多语言编码" field="code" align="left">
        </vxe-table-column>
        ...
        <vxe-table-column  title="类型" field="type" align="center">
              <template slot-scope="scope">
                ...
                <el-tag v-else-if="scope.row.type === '3'" size="mini" type="info">路由</el-tag>
              </template>
        </vxe-table-column>
        <vxe-table-column title="排序号" field="sort" width="170px" align="center">
            <template slot-scope="scope">
                  <el-input-number size="mini" v-model="scope.row.sort" @change="sortChange(scope.row)" :step="30"  :min="0" :max="10000" label="描述文字"></el-input-number>
                </template>
        </vxe-table-column>
        ...
        <vxe-table-column title="操作" width="150px"  align="center">
          <template slot-scope="scope">
                  <el-button  type="text" size="mini" @click="showRight(scope.row)">数据规则</el-button>
