    ...
    <div class="jp-common-layout page">
      ...
      <el-form size="mini" :inline="true" class="query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
         <el-form-item prop="loginName">
          <el-input size="mini" v-model="searchForm.loginName" placeholder="登录名" clearable></el-input>
         </el-form-item>
      ...
      <el-dialog  title="导入Excel" :visible.sync="isImportCollapse">
        <el-form size="mini" :inline="true" ref="importForm">
          <el-form-item>
      ...
      <el-button v-if="hasPermission('sys:user:del')" type="danger"   size="mini" icon="el-icon-delete" @click="del()"
        ...
        <el-button-group class="pull-right">
          <el-tooltip class="item" effect="dark" content="导入" placement="top">
              <el-button v-if="hasPermission('sys:user:import')" type="default" size="mini" icon="el-icon-upload2" title="导入" @click="isImportCollapse = !isImportCollapse"></el-button>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="导出" placement="top">
            <el-button v-if="hasPermission('sys:user:export')" type="default" size="mini" icon="el-icon-download" title="导出" @click="exportExcel()"></el-button>
          </el-tooltip>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <img :src="scope.row.photo === ''?'/static/img/avatar.png':scope.row.photo" style="height:28px"/>
        </template>
      </el-table-column>
      <el-table-column
        prop="loginName"
      ...
      <el-table-column
        ...
        label="状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.loginFlag === '1'" size="mini" type="success">正常</el-tag>
          <el-tag v-else-if="scope.row.loginFlag === '0'" size="mini" type="danger">禁用</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button v-if="hasPermission('sys:user:view')" type="text" size="mini" icon="el-icon-view" @click="view(scope.row.id)">查看</el-button>
          <el-button v-if="hasPermission('sys:user:edit')" type="text" size="mini" icon="el-icon-edit" @click="edit(scope.row.id)">修改</el-button>
          <el-button v-if="hasPermission('sys:user:del')" type="text" size="mini" icon="el-icon-delete" @click="del(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
