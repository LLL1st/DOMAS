      ...
      <div v-if="editForm.opType==='update' && editForm.modelvalue !== undefined">
        ...
        <el-form :model="editForm" :rules="editFormRules" ref="editupdateTable" :inline="true">
          ...
          align="center"
                ...
                <template slot-scope="scope">
                  <!--                <el-button-->
                  <!--                    @click.native.prevent="editRow(scope.$index, tableData1)"-->
                  <!--                    type="text"-->
                  <!--                    size="small">-->
                  <!--                  编辑-->
                  <!--                </el-button>-->
...
<!--      查询-->
      <div v-if="editForm.opType==='select' && editForm.modelvalue !== undefined">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!--        <el-button @click="dialogVisible = true">上一步</el-button>-->
        ...
        <el-button v-if="editForm.opType==='select'" type="primary" @click="submitquery()">确 定</el-button>
      </div>
    </el-dialog>
<!--    编辑框-->
    <el-dialog
...
<!--          <b-code-editor v-model="editViewForm.filter" :auto-format="true"  :smart-indent="true" height=200px  :indent-unit="4" :gutter="false" ref="editor"></b-code-editor>-->
    ...
    <el-dialog
      ...
      <el-form :model="editConditionForm" :rules="editFormRules" ref="editConditionStringForm" label-position="left" size="middle" label-width="100px">
        <el-form-item label="字段名称" prop="name">
          <!--          <el-input v-model="editConditionForm.name" autocomplete="off" ></el-input>-->
          <el-select
              v-model="editConditionForm.name"
              @change="editConditionFormType(editConditionForm.name)"
              placeholder="请输入关键词">
            <el-option
        ...
        <el-form-item prop="type" label=" 字段类型">
          <el-input disabled="true" v-model="editConditionForm.type" style="width: 65%"></el-input>
        </el-form-item>
        <div v-if="editConditionForm.type!=='int'">
          <el-form-item :required="true" label="操作条件" :rules="editFormRules">
            <div style="text-align: left">
...
import {delModelById, delModelByIds, getAllModel, idModelVo} from "@/api/model";
...
export default {
  ...
  methods: {
    editConditionFormType(name){
      for(var i = 0; i < this.tableData1.length; i++){
        if (this.tableData1[i].name == name){
          this.editConditionForm.type = this.tableData1[i].type
          break
        }
      }
    },
    ...
    updateApi(formName){
      if (!this.isJSON(this.editViewForm.filter)){
        ...
        if (formName !== null) {
          console.log('==============')
          console.log(this.editViewForm.accessUser)
          const data = {
            query:{
    ...
    getApiList() {
      ...
      page(this.searchForm).then(res => {
        ...
        //     if (this.tableData[i].accessUser == this.useroptions[j].value){
        //       this.tableData[i].accessuser = this.useroptions[j].label
        //     }
        //   }
        // }
      })
